...
::: {.section-block}
### 📐 MIT 18.065: Linear Algebra Applications <span class="section-count">12 lectures</span>
...
::: {.section-block}
### 📐 Stanford EE 364A: Convex Optimization <span class="section-count">10 lectures</span>
...
::: {.content-card}
**[Chapter 4.3: Linear Optimization](Math/EE364A/ee364a-lecture8-linear-optimization.qmd)**
Linear programs (LP) minimize linear objectives subject to affine constraints. Geometric interpretation: level curves orthogonal to objective vector $c$ touch polyhedron feasible set. Applications: diet problem (minimize cost meeting nutritional requirements), piecewise-linear minimization (epigraph formulation $\min t$ s.t. $a_i^\top x+b_i\le t$), Chebyshev center (largest inscribed ball), linear fractional programs (convert to LP via $y=xz$, $z=1/(e^\top x+f)$). Von Neumann growth model: maximize minimum sectoral growth rate using quasiconvex bisection.
:::
