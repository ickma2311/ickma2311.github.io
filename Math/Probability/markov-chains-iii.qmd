...
<figure>
<img id="capacity-animation" src="markov-iii-assets/capacity.png" alt="Exact Erlang loss model: 90 lines block 7.957 percent of arrivals, 106 lines block 1.055 percent, and 107 lines block 0.880 percent. The animation compares capacities, not a time trajectory." width="1600" height="900" style="width:100%;height:auto">
...
These formulas apply where the neighboring state exists. At $B$, rejected arrivals do not increase the state; at zero, no departure is possible. Multiple events contribute higher-order terms. The lecture uses this small-time construction to obtain the continuous-time birth–death model.
...
Here eventual absorption occurs with probability one, and $I-Q$ is invertible. Do not apply this conclusion blindly if an omitted recurrent class can retain the process indefinitely.
...
As a separate extension, time to either absorbing outcome in the original graph satisfies $(I-Q)t=\mathbf1$, giving about 9.46 steps from state 1. That is also different from the conditional time to reach 4 given that 4 is reached.
...
It requires at least one transition, even if that transition is a self-loop. First-step analysis gives

$$
t_s^+=1+\sum_jp_{sj}t_j.
$$

![A worked comparison for the two-state chain: first hitting state 1 from state 1 takes zero steps; hitting from state 2 takes five expected steps; returning to state 1 takes 1 plus 0.5 times zero plus 0.5 times five, or 3.5 expected steps.](markov-iii-assets/hitting-return.png)
...
As a consistency check, the finite irreducible chain's mean return time is $1/\pi_1=7/2$. A state occupied more often in the long run has a shorter mean recurrence interval. Do not confuse this recurrence statement with independence of consecutive states.
